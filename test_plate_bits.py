...
from plate_bits import BITONAL_DIR_NAME, is_bitonal_plate, is_prescreened_plate, save_bitonal_plate
...
class PlateBitsTests(unittest.TestCase):
    ...
    def test_limited_tone_plate_is_not_forced_to_one_bit(self) -> None:
        ...
        with tempfile.TemporaryDirectory(dir=temp_root) as tmp:
            ...
            self.assertTrue(is_prescreened_plate(Image.fromarray(arr)))
            self.assertFalse(is_bitonal_plate(Image.fromarray(arr)))
            self.assertIsNone(save_bitonal_plate(source, "limited"))
            gc.collect()
